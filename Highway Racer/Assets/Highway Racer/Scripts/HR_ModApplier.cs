	...
	void CreateSiren(){
		...
		for (int i = 0; i < 2; i++) {
			...
			if (frontSirenLights [i].transform.localPosition.x >= 0f) {
				frontSirenLights [i].transform.position = GetComponent<RCC_CarControllerV3> ().headLights [i].transform.position;
		...
		for (int i = 0; i < 2; i++) {
			...
			if (rearSirenLights [i].transform.localPosition.x >= 0f) {
				rearSirenLights [i].transform.position = GetComponent<RCC_CarControllerV3> ().brakeLights [i].transform.position;
